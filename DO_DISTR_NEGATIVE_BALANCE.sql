                        ...
                        FROM
                            ...
                            TABLE ( CAST(MULTISET(
                                ...
                                FROM
                                    TABLE(ttab#number(0) )
                                CONNECT BY
                                    level <= c#next_mn+1 - c#mn -- +1 добавлено 21.03.2018 чтобы переплата разносилась в том числе и на текущий мес€ц
                            ) AS ttab#number) ) t
                    ) a,
                    TABLE ( CAST(MULTISET(
                        SELECT
    ...
    FOR c IN pay LOOP
        BEGIN
            ...
            INSERT INTO t#ops_vd (
                ...
                kind_id,
                CASE
                    WHEN c.c#npd IS NOT NULL THEN '(Ќѕƒ ' || c.c#npd || ')'
                END
            );
 ...
 -- commit;
                ...
                THEN
                    dbms_output.put_line(c.c#id
                     || ' ќстаток после всех распределений = '
                     || ostatok);
